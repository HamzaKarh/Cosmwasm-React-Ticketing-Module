...
// use crate::msg::QueryMsg::GetOwner;
...
pub fn instantiate(
    ...
    Ok(Response::new()
        ...
        // .add_attribute("count", msg.owner.to_string()))
}
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::Transfer { target } => try_transfer(deps, info, target)
        // ExecuteMsg::Increment {} => try_increment(deps),
        // ExecuteMsg::Reset { count } => try_reset(deps, info, count),
    }
}

pub fn try_transfer(deps: DepsMut, info: MessageInfo, target: Addr) -> Result<Response, ContractError>{
...
mod tests {
    ...
    fn transfer() {
        ...
        let creator = info.clone();
        let _res = instantiate(deps.as_mut(), mock_env(), info, msg).unwrap();

        // beneficiary can release it
        let target = mock_info("anyone", &coins(2, "token"));
        let msg = ExecuteMsg::Transfer{ target: target.sender };
        let _res = execute(deps.as_mut(), mock_env(), creator, msg).unwrap();

        // should increase counter by 1
        let res = query(deps.as_ref(), mock_env(), QueryMsg::GetCount {}).unwrap();
        let value: GetOwnerResponse = from_binary(&res).unwrap();
        assert_eq!(target.sender, value.owner);
    }
